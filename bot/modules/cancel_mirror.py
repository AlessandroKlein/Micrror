...
def cancel_mirror(update, context):
    ...
    if len(args) > 1:
        ...
        if not dl:
            sendMessage(f"GID: <code>{gid}</code> No encontrado.", context.bot, update)
            return
        ...
        with download_dict_lock:
            ...
            try:
                ...
                pass
    if len(args) == 1:
        msg = f"Por favor responda al <code>/{BotCommands.MirrorCommand}</code> mensaje que se utilizó para iniciar la descarga o enviar <code>/{BotCommands.CancelMirror} GID</code> para cancelarlo!"
        if mirror_message and mirror_message.message_id not in keys:
            if BotCommands.MirrorCommand in mirror_message.text or \
               ...
               BotCommands.UnzipMirrorCommand in mirror_message.text:
                msg1 = "Mirror ya ha sido cancelado"
    ...
    if dl.status() == MirrorStatus.STATUS_ARCHIVING:
        sendMessage("Archivado en curso, no puede cancelarlo.", context.bot, update)
    elif dl.status() == MirrorStatus.STATUS_EXTRACTING:
        sendMessage("Extracto en progreso, no puede cancelarlo.", context.bot, update)
    else:
...
def cancel_all(update, context):
    ...
    while True:
        ...
        if dl:
            ...
            break
    sendMessage(f'{count} Descarga(s) ha sido cancelado!', context.bot, update)
